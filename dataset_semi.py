...
class SatMapDataset(Dataset):
    def __init__(self, config, is_train, dev_run=False, osm_masks_dir=None, use_semi_supervised=False, inference_masks_dir=None):
        self.config = config
        self.use_semi_supervised = use_semi_supervised
        
        assert self.config.DATASET in {'cityscale', 'spacenet', 'ekb2048', 'combined'}
        ...
        elif self.config.DATASET == 'ekb2048':
            self.IMAGE_SIZE = 2048
            self.SAMPLE_MARGIN = 64

            rgb_pattern = './ekb/2048/X1/{}.png'
            osm_mask_pattern = './ekb2048/osm_masks/osm_mask_{}.png'
            inference_mask_pattern = './ekb2048/inference_masks/inference_mask_{}.png'

            # Екатеринбург разделение данных
            train, val, test = ekb_data_partition_2048()

            # coord-transform = (r, c) -> (x, y)
            coord_transform = lambda v: v[:, ::-1]

        elif self.config.DATASET == 'combined':
            cityscale_train, cityscale_val, cityscale_test = cityscale_data_partition()
            ekb_train, ekb_val, ekb_test = ekb_data_partition_2048()

            # Объединяем train, val, test
            train = cityscale_train + ekb_train
            val = cityscale_val + ekb_val
            test = cityscale_test + ekb_test

            # Определяем шаблоны файлов
            cityscale_rgb_pattern = './cityscale/20cities/region_{}_sat.png'
            ekb_rgb_pattern = './ekb/2048/X1/{}.png'

            cityscale_road_mask_pattern = './cityscale/processed/road_mask_{}.png'
            ekb_osm_mask_pattern = './ekb2048/osm_masks/osm_mask_{}.png'
            ekb_inference_mask_pattern = './ekb2048/inference_masks/inference_mask_{}.png'

            # Условные пути для разных датасетов
            def get_patterns(tile_idx):
                if tile_idx in cityscale_train + cityscale_val + cityscale_test:
                    return (
                        './cityscale/20cities/region_{}_sat.png',
                        './cityscale/processed/road_mask_{}.png',
                        None,  # OSM не используется в Cityscale
                        None  # Inference не используется в Cityscale
                    )
                else:
                    return (
                        './ekb/2048/X1/{}.png',
                        None,  # Road mask отсутствует для Екатеринбурга
                        './ekb2048/osm_masks/osm_mask_{}.png',
                        './ekb2048/inference_masks/inference_mask_{}.png'
                    )

            # # Используем логику объединения
            # def get_pattern(tile_idx):
            #     if tile_idx in cityscale_train or tile_idx in cityscale_val or tile_idx in cityscale_test:
            #         return (cityscale_rgb_pattern, cityscale_road_mask_pattern)
            #     else:
            #         return (ekb_rgb_pattern, ekb_osm_mask_pattern, ekb_inference_mask_pattern)
        ...
        # Загрузка данных
        self.rgbs, self.road_masks, self.osm_masks, self.inference_masks = [], [], [], []
        for tile_idx in train_split + val_split + test_split:
            rgb_pattern, road_mask_pattern, osm_mask_pattern, inference_mask_pattern = get_patterns(tile_idx)

            self.rgbs.append(read_rgb_img(rgb_pattern.format(tile_idx)))

            # Road Mask
            if road_mask_pattern:
                self.road_masks.append(cv2.imread(road_mask_pattern.format(tile_idx), cv2.IMREAD_GRAYSCALE))
            else:
                self.road_masks.append(None)

            # OSM Mask
            if osm_mask_pattern:
                self.osm_masks.append(cv2.imread(osm_mask_pattern.format(tile_idx), cv2.IMREAD_GRAYSCALE))
            else:
                self.osm_masks.append(None)

            # Inference Mask
            if inference_mask_pattern:
                self.inference_masks.append(cv2.imread(inference_mask_pattern.format(tile_idx), cv2.IMREAD_GRAYSCALE))
            else:
                self.inference_masks.append(None)
        
        # # OPEN STREET MAP
        # self.osm_masks = []
        # if osm_masks_dir:
        #     for tile_idx in tile_indices:
        #         osm_mask_path = os.path.join(osm_masks_dir, f"osm_mask_{tile_idx}.png")
        #         if os.path.exists(osm_mask_path):
        #             self.osm_masks.append(cv2.imread(osm_mask_path, cv2.IMREAD_GRAYSCALE))
        #         else:
        #             self.osm_masks.append(None)
        #
        # self.inference_masks = []
        # if inference_masks_dir:
        #     for tile_idx in tile_indices:
        #         inference_mask_path = os.path.join(inference_masks_dir, f"inference_mask_{tile_idx}.png")
        #         if os.path.exists(inference_mask_path):
        #             self.inference_masks.append(cv2.imread(inference_mask_path, cv2.IMREAD_GRAYSCALE))
        #         else:
        #             self.inference_masks.append(None)
    ...
    def __getitem__(self, idx):
        ...
        # OPEN STREET MAP
        osm_mask = self.osm_masks[img_idx]
        if self.use_semi_supervised and osm_mask is not None:
            osm_patch = osm_mask[begin_y:end_y, begin_x:end_x]
        else:
            osm_patch = np.zeros_like(road_mask_patch)

        inference_mask = self.inference_masks[img_idx]
        if self.use_semi_supervised and inference_mask is not None:
            inference_patch = inference_mask[begin_y:end_y, begin_x:end_x]
        else:
            inference_patch = np.zeros_like(road_mask_patch)
        ...
        return {
            ...
            'osm_mask': torch.tensor(osm_patch, dtype=torch.float32) / 255.0,
            'inference_mask': torch.tensor(inference_patch, dtype=torch.float32) / 255.0,
        }
