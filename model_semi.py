...
class SAMRoad(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # OPEN STREET MAP Фильтруем loss только по областям с разметкой
        if self.config.get('USE_SEMI_SUPERVISED', False):
            self.log("semi_supervised", True, prog_bar=True)

            # OSM-маска (учёт только размеченных пикселей)
            # road_loss_mask = (osm_mask > 0).float()
            if osm_mask is not None:
                road_loss_mask = (osm_mask > 0).float()  # OSM как ground truth
            elif inference_mask is not None:
                road_loss_mask = (inference_mask > 0).float()  # Инференс как ground truth
            else:
                road_loss_mask = torch.ones_like(road_mask)  # Cityscale — все пиксели ground truth

            # Инференс-маска (дополнение разметки)
            additional_mask = (inference_mask > 0).float()

            # Объединённая маска (учитываем OSM и инференс)
            combined_loss_mask = torch.maximum(road_loss_mask, additional_mask)

            # Считаем потери только в размеченных областях
            mask_loss = self.mask_criterion(mask_logits, gt_masks)
            mask_loss = mask_loss * combined_loss_mask.unsqueeze(-1)  # Учитываем только размеченные пиксели
            mask_loss = mask_loss.sum() / combined_loss_mask.sum()  # Нормализация
        else:
            self.log("semi_supervised", False, prog_bar=True)
            mask_loss = self.mask_criterion(mask_logits, gt_masks)
